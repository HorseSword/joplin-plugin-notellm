...
export function getTxt(lan='en') {
    ...
    //
    dictText['prompt_chat']=isZH?'你是用户的助手。你的任务是以对话的方式，基于用户前文提供的信息，以对话的形式回复最后的段落。请注意，回复完成之后不要额外追问。':`You are helpful assistant. Your task is to respond to the final paragraph in a conversational manner based on the information provided by the user previously. Please note that you should not ask additional follow-up questions after your response.`;
    dictText['prompt_summary']=isZH?'请简要概括上文的主要内容，并用列表的方式列举提炼出的要点。':'Please briefly summarize the main content of the above text and list the key points extracted from it.'
