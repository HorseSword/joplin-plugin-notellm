...
export function getTxt(lan='en') {
    ...
    dictText['ask_label'] = isZH?'提问选中部分（考虑上下文）':'Ask about selection (based on whole note)'
    dictText['improve_label'] = isZH?'改写选中部分（考虑上下文）':'Rewrite selection (based on before and after)'
    //
    // settings
